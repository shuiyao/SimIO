...
void write_header(struct dump *, FILE *);
#endif
...
#ifdef MARK64
			printf("Reading for 64 machine! \n");
			read_header(&header1, fd);
#else
			printf("Reading for 32 machine! \n");
...
#ifdef MARK64
		printf("Writing for 64 machine.\n");
   		write_header(&header,outfile); /* Mark Fardal's header writing function, writes 32-bit-style headers on 64-bit machines */
#else		
		printf("Writing for 32 machine.\n");
...
/* Set some cosmology units stuff to physical units */ 
  ...
  /* fix header if gas particles not output due to density/temp criterion */
  		if( header.nsph != ngasout ) {
			...
			header.nsph = ngasout;
			header.nstar = nstarout;
 			header.nbodies = header.nsph+header.ndark+header.nstar;
			rewind(outfile);
#ifdef MARK64
			printf("Writing for 64 machine.\n");
			write_header(&header,outfile);	 
#else
			printf("Writing for 32 machine.\n");
  			fwrite(&header, sizeof(header), 1, outfile);
#endif
...
int output_tipsy_gas(FILE *outfile, FILE *idfile, FILE *auxfile)
{
  int   i;
  short int   Nrec;
...
/* 	  if(i == 10000) printf("%g\n", P[i].Vel[0]); */
	  ...
	  gasp.rho = P[i].Rho;
	  gasp.metals = P[i].metal[0]+P[i].metal[1]+P[i].metal[2]+P[i].metal[3];
	  gasp.metals *= 0.0189/0.0147;  // correct to total solar metal mass
	  // solar frac used in sims: C=0.00307, O=0.009618, Si=0.0007109, Fe=0.001267
	  gasp.phi = P[i].Phi;	/* we now store potential (not SFR) in Phi */
	  if( gasp.rho < mindens || gasp.rho > maxdens || gasp.temp < minT || gasp.temp > maxT || gasp.hsmooth <= 0 || gasp.hsmooth > 1){
	  //if( gasp.rho < mindens || gasp.rho > maxdens || gasp.temp < minT || gasp.temp > maxT){
	    fprintf(stderr,"Corrupted density = %g or temperature = %g or smoothing length = %g\n",gasp.rho, gasp.temp, gasp.hsmooth);
	    continue;
	  }
	  ...
	  //if((P[i].Tmax<10 && bNoWindMode) || (bNoWindMode == 0)){
	    fwrite(&gasp, sizeof(struct gas_particle), 1, outfile);
	    if( id_flag ) fwrite(&Id[i], sizeof(int), 1, idfile) ;
	    fwrite(P[i].metal, sizeof(float), NMETALS, auxfile) ;
	    fwrite(&P[i].Sfr, sizeof(float), 1, auxfile) ;
	    fwrite(&P[i].Tmax, sizeof(float), 1, auxfile) ;
	    fwrite(&P[i].DelayTime, sizeof(float), 1, auxfile) ;
	    fwrite(&P[i].Ne, sizeof(float), 1, auxfile) ;
	    fwrite(&P[i].Nh, sizeof(float), 1, auxfile) ;
	    fwrite(&P[i].Nspawn, sizeof(int), 1, auxfile) ;
#ifdef WRITE_NREC
	    Nrec = -1;
	    fwrite(&Nrec, sizeof(short int), 1, auxfile) ;	 
	    fwrite(&Nrec, sizeof(short int), 1, auxfile) ; // PAD   
#endif
	    ngasout++ ;
	    //}
    }
...
int output_tipsy_star(FILE *outfile, FILE *idfile, FILE *auxfile)
{
  int   i;
  short int Nrec;
  for(i=1; i<=NumPart; i+=nskip) {
	  ...
	  starp.metals = P[i].metal[0] + P[i].metal[1] + P[i].metal[2] + P[i].metal[3];
	  starp.metals *= 0.0189/0.0147;  // correct to total solar metal mass
	  // solar frac used in sims: C=0.00307, O=0.009618, Si=0.0007109, Fe=0.001267
	  ...
	  starp.phi = P[i].Phi;
	    ...
	    if( id_flag ) fwrite(&Id[i], sizeof(int), 1, idfile) ;
	    fwrite(P[i].metal, sizeof(float), NMETALS, auxfile) ;
	    fwrite(&P[i].age, sizeof(float), 1, auxfile) ;
	    fwrite(&P[i].Tmax, sizeof(float), 1, auxfile) ;
	    fwrite(&P[i].Nspawn, sizeof(int), 1, auxfile) ;
#ifdef WRITE_NREC
	    Nrec = -1;
